...
#pragma omp barrier
  ...
  idx_t nnz = A->rowptr[A->m] - BASE;
  idx_t nnzPerThread = (nnz + nthreads - 1)/nthreads;
  int xBegin, xEnd;
  if (WITH_BIT_VECTOR) {
    xBegin = iBegin;
    xEnd = iEnd;
  ...
  for (int x = xBegin; x < xEnd; ++x) {
    ...
    for (idx_t j = AT->rowptr[xx] - BASE; j < AT->rowptr[xx + 1] - BASE; ++j) {
      int y = AT->colidx[j];
      assert(!WITH_BIT_VECTOR || !bv->get(y));
      int xx2 = xx + A->m;
      if (p[xx2] != p[y]) {
  ...
  int *nVisited1, int *nVisited2)
{
  ...
  while (true) {
    ...
    for (int i = 0; i < qTail; ++i) {
      ...
      for (idx_t j = AT->rowptr[u] - base; j < AT->rowptr[u + 1] - base; ++j) {
...
#pragma omp master
    ...
    idx_t *rowptr = rowptrs + tid*max(A->m, A->n);
    *rowptr = 0;
