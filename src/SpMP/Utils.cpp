...
template<typename T>
void getLoadBalancedPartition_(int *begin, int *end, const T *prefixSum, T n)
{
  int nthreads = omp_get_num_threads();
  int tid = omp_get_thread_num();
...
{
  return getLoadBalancedPartition_(begin, end, prefixSum, n);
}

void getLoadBalancedPartition(int *begin, int *end, const idx_t *prefixSum, idx_t n)
{
  return getLoadBalancedPartition_(begin, end, prefixSum, n);
}
