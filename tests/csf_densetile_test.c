...
CTEST_SETUP(csf_tile)
{
  data->tt = tt_read(DATASET(med4.tns));
}

CTEST_TEARDOWN(csf_tile)
{
  tt_free(data->tt);
}

CTEST2(csf_tile, csf_one_densefill5)
{
...
CTEST2(csf_tile, csf_one_densefill1)
{
  double * opts = splatt_default_opts();
  opts[SPLATT_OPTION_CSF_ALLOC] = SPLATT_CSF_ONEMODE;
  opts[SPLATT_OPTION_TILE]      = SPLATT_DENSETILE;
...
CTEST2(csf_tile, csf_one_ccpfill5)
{
  double * opts = splatt_default_opts();
  opts[SPLATT_OPTION_CSF_ALLOC] = SPLATT_CSF_ONEMODE;
  ...
  /* compare vals and inds */
  idx_t * nnzptr = tt_ccptile(data->tt, cs->tile_dims);
  for(idx_t t=0; t < cs->ntiles; ++t) {
    ASSERT_EQUAL(nnzptr[t+1] - nnzptr[t], cs->pt[t].nfibs[cs->nmodes-1]);
...
CTEST2(csf_tile, csf_one_ccpfill1)
{
  double * opts = splatt_default_opts();
  opts[SPLATT_OPTION_CSF_ALLOC] = SPLATT_CSF_ONEMODE;
  opts[SPLATT_OPTION_TILE]      = SPLATT_CCPTILE;
